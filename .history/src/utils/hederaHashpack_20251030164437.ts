...
export function useSkyRunActions() {
  ...
  async function getAvailableLives() {
    ...
    try {
      ...
      // Get the real Hedera contract ID first
      const hederaContractId = await getHederaContractId(SKYRUN_ADDRESSES.game);
      const contractAddress = hederaContractId || SKYRUN_ADDRESSES.game;
      
      console.log('Querying mirror node with contract address:', contractAddress);
      
      const response = await fetch(`https://testnet.mirrornode.hedera.com/api/v1/contracts/${contractAddress}/call`, {
        method: 'POST',
