...
export function useSkyRunActions() {
  ...
  async function getAvailableLives(): Promise<number> {
    ...
    try {
      ...
      const iface = new ethers.utils.Interface(["function getAvailableLives() view returns (uint256)"]);
      ...
      const val = decoded[0];
      // decoded element may be BigNumber
      return Number(ethers.BigNumber.from(val).toString());
    } catch (err) {
  ...
  async function getTokenBalance(): Promise<number> {
    ...
    try {
      const evmAddress = await accountIdToEvmAddress(accountId);
      const contractEvmAddress = SKYRUN_ADDRESSES.game;
      const iface = new ethers.utils.Interface(["function getTokenBalance() view returns (uint256)"]);
      ...
      return Number(ethers.BigNumber.from(decoded[0]).toString());
    } catch (err) {
