...
export function useSkyRunActions() {
  ...
  async function getAvailableLives(): Promise<number> {
    ...
    try {
      // PRIMARY: Use Hedera SDK (works immediately, even for new contracts)
      const { ContractCallQuery, ContractFunctionParameters, Hbar } = await import('@hashgraph/sdk');
      const contractId = evmToContractId(SKYRUN_ADDRESSES.game);
      
      console.log('📖 Reading getAvailableLives via Hedera SDK...');
      
      const query = new ContractCallQuery()
        .setContractId(contractId)
        .setGas(100_000)
        .setQueryPayment(new Hbar(0.05)) // Explicit payment (no operator set)
        .setFunction('getAvailableLives', new ContractFunctionParameters());
      ...
      console.warn('SDK query failed, trying Mirror Node fallback...', sdkError);
      
      // FALLBACK: Try Mirror Node (may fail for newly deployed contracts)
      try {
        ...
        const json = await mirrorNodeCallWithRetry(contractEvmAddress, data, evmAddress, 10000, 2000);
        ...
        console.warn('Mirror Node also failed - contract may not be indexed yet. Returning 0.');
        return 0;
      }
    }
  }
  ...
  async function getTokenBalance(): Promise<number> {
    ...
    try {
      // PRIMARY: Use Hedera SDK (works immediately)
      const { ContractCallQuery, ContractFunctionParameters, Hbar } = await import('@hashgraph/sdk');
      const contractId = evmToContractId(SKYRUN_ADDRESSES.game);
      
      console.log('📖 Reading getTokenBalance via Hedera SDK...');
      
      const query = new ContractCallQuery()
        .setContractId(contractId)
        .setGas(100_000)
        .setQueryPayment(new Hbar(0.05)) // Explicit payment (no operator set)
        .setFunction('getTokenBalance', new ContractFunctionParameters());
      ...
      try {
        ...
        console.warn('Mirror Node also failed - returning 0.');
        return 0;
      }
    }
  }
  ...
  async function getUserStats() {
    ...
    try {
      ...
      console.log('📖 Reading getUserStats via Hedera SDK...');
      
      const query = new ContractCallQuery()
        .setContractId(contractId)
        .setGas(100_000)
        .setFunction('getUserStats', new ContractFunctionParameters());
      
      const result = await query.execute(client);
      
      // Decode the 7 uint256 values from the Stats struct
      const stats = {
      ...
      try {
        ...
        if (!hex) {
          console.warn('No result from Mirror Node');
          return defaultStats;
        }
        ...
        console.log("📊 Fetched user stats from Mirror Node:", stats);
        return stats;
      } catch {
        console.warn('Mirror Node also failed - returning defaults.');
        return defaultStats;
      }
    }
  }
