...
// Helper to get Hedera contract ID from HashScan API
async function getHederaContractId(evmAddress: string): Promise<string | null> {
  try {
    // Query HashScan/Mirror Node to get the Hedera contract ID
    const response = await fetch(`https://testnet.mirrornode.hedera.com/api/v1/contracts/${evmAddress}`);
    if (response.ok) {
      const data = await response.json();
      if (data.contract_id) {
        return data.contract_id; // Returns format like "0.0.xxxxx"
      }
    }
  } catch (error) {
    console.warn('Could not fetch Hedera contract ID from mirror node:', error);
  }
  return null;
}
...
export function useSkyRunActions() {
  ...
  async function getAvailableLives() {
    ...
    try {
      ...
      // Try mirror node with EVM address directly first
      let response = await fetch(`https://testnet.mirrornode.hedera.com/api/v1/contracts/${SKYRUN_ADDRESSES.game}/call`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ data, estimate: false })
      });
      
      // If 404, the contract might not be indexed yet or we need the Hedera ID
      if (!response.ok) {
        console.warn('Mirror node query failed with EVM address, trying contract ID format...');
        // Try converting to contract ID format as fallback
        const contractId = evmToContractId(SKYRUN_ADDRESSES.game);
        const contractIdString = `${contractId.shard}.${contractId.realm}.${contractId.num}`;
        response = await fetch(`https://testnet.mirrornode.hedera.com/api/v1/contracts/${contractIdString}/call`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ data, estimate: false })
        });
      ...
      console.warn('Could not get available lives from mirror node:', result);
      return 0;
    } catch (error) {
