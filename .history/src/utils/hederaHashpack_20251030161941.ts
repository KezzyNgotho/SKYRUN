...
export function useSkyRunActions() {
  ...
  async function getAvailableLives() {
    ...
    try {
      ...
      // Call getAvailableLives(address) using mirror node
      const functionSelector = '0x' + Buffer.from('getAvailableLives(address)').toString('hex').slice(0, 8);
      const paddedAddress = evmAddress.replace('0x', '').padStart(64, '0');
      ...
      if (result.result) {
        const lives = parseInt(result.result.slice(-64), 16);
        return lives;
      }
      return 0;
    } catch (error) {
  ...
  async function getTokenBalance() {
    ...
    try {
      ...
      const query = new ContractCallQuery()
        .setContractId(contractId)
        .setGas(100_000)
        .setMaxQueryPayment(new Hbar(1)) // Allow up to 1 HBAR for query (usually costs much less)
