...
export function useSkyRunActions() {
   ...
   * - Uses Mirror Node /call endpoint (required for msg.sender pattern)
   * - Returns 0 silently if contract not indexed yet
   */
   ...
   * READ: getUserStats
   * - Uses Mirror Node /call endpoint (required for msg.sender pattern)
   * - Returns default stats silently if contract not indexed yet
   */
