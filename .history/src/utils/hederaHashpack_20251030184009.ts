    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.info('⏰ Contract not indexed yet by Mirror Node (takes 2-5 minutes after deployment)');
          console.info('💡 Your transaction was successful! Stats will load automatically once indexed.');
        } else {
  ...
  async function getUserStats() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.info('⏰ Contract not indexed yet by Mirror Node (takes 2-5 minutes after deployment)');
          console.info('💡 Your data is safely stored on-chain! It will appear once Mirror Node finishes indexing.');
        }
        return defaultStats;
      }
