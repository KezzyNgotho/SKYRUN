import { ContractExecuteTransaction, ContractFunctionParameters, ContractId, Client, Hbar, AccountId, TransactionId, Duration } from '@hashgraph/sdk';
...
export function useSkyRunActions() {
  ...
  async function submitScore(score: number) {
    ...
    // Create transaction with TransactionId and extended validity duration
    const tx = new ContractExecuteTransaction()
      ...
      .setTransactionValidDuration(Duration.fromMinutes(5)); // Extended validity for wallet approval
  ...
  async function buyLifeLine() {
    ...
    // Create transaction with TransactionId (required for freezeWith)
    const tx = new ContractExecuteTransaction()
      .setContractId(contractId)
      .setGas(350_000)
