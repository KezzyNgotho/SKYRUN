...
export function useSkyRunActions() {
  ...
  async function getAvailableLives(): Promise<number> {
    ...
    try {
      ...
      const query = new ContractCallQuery()
        .setContractId(contractId)
        .setGas(100_000)
        .setQueryPayment(new Hbar(0.1)) // Explicit payment (no operator needed)
  ...
  async function getTokenBalance(): Promise<number> {
    ...
    try {
      const { ContractCallQuery, ContractFunctionParameters, AccountId, Hbar } = await import('@hashgraph/sdk');
      ...
      const query = new ContractCallQuery()
        .setContractId(contractId)
        .setGas(100_000)
        .setQueryPayment(new Hbar(0.05)) // Explicit payment (client has no operator)
