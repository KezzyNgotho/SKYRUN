import { ContractExecuteTransaction, ContractFunctionParameters, ContractId, Client, Hbar, AccountId, TransactionId } from '@hashgraph/sdk';
...
export function useSkyRunActions() {
  ...
  async function submitScore(score: number) {
    ...
    // Create transaction WITHOUT freezing or setting TransactionId
    // freezeWithSigner will handle both freezing and setting the transaction ID
  ...
  async function buyLifeLine() {
    ...
    // Create transaction WITHOUT freezing - HashConnect will handle freezing with freezeWithSigner
    const tx = new ContractExecuteTransaction()
      .setContractId(contractId)
      .setGas(350_000)
