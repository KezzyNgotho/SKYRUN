...
export function useSkyRunActions() {
  ...
  async function getUserStats() {
    ...
    try {
      ...
      // Create a new client without operator for read-only queries
      const queryClient = Client.forTestnet();
      queryClient.setMaxQueryPayment(new Hbar(2)); // Set max payment client is willing to pay
      ...
      const cost = await query.getCost(queryClient);
      console.log('Query cost:', cost.toString());
      
      // Execute with the exact cost
      const result = await query.setQueryPayment(cost).execute(queryClient);
