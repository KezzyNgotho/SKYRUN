    ...
    sendTransaction: async (txBytesOrTransaction: TransactionInput) => {
      ...
      try {
        ...
        try {
          ...
          // Step 4: Use signer to sign the bytes (simpler approach)
          console.log('Step 4: Signing transaction bytes with signer...');
          
          // The signer object from HashConnect v3 can sign bytes directly
          // This will prompt HashPack for approval
          const hc = hashconnect as any; // eslint-disable-line @typescript-eslint/no-explicit-any
          
          // Try to use the request method with proper formatting
          console.log('Attempting to sign via HashConnect request...');
          
          const signResponse = await hc.request({
            topic: '', // Will be filled by HashConnect internally
            request: {
              method: 'hedera_signAndExecuteTransaction',
              params: {
                transactionList: btoa(String.fromCharCode(...txBytes))
              }
            }
          });
          
          console.log('✅ Transaction signed and executed via HashConnect!');
