  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytesOrTransaction: TransactionInput) => {
      ...
      try {
        ...
        // Try SIMPLER approach: Just call methods directly on transaction with signer
        let result;
        
        try {
          console.log('🔍 Exploring signer methods for direct execution...');
          
          // Check if signer can be used directly with transaction.executeWithSigner
          if (typeof transaction.executeWithSigner === 'function') {
            console.log('Method 1: Using transaction.executeWithSigner(signer)...');
            result = await transaction.executeWithSigner(signer);
            console.log('✅ Transaction executed with signer!');
          }
          // Try using the signer's call method with transaction
          else if (typeof signer.call === 'function') {
            console.log('Method 2: Using signer.call(transaction)...');
            result = await signer.call(transaction);
            console.log('✅ Transaction called via signer!');
          }
          // Try HashConnect's request method if it exists
          else if (typeof (hashconnect as any).request === 'function') { // eslint-disable-line @typescript-eslint/no-explicit-any
            console.log('Method 3: Using hashconnect.request...');
            ...
            console.log('✅ Transaction executed via hashconnect.request!');
          }
          // Last resort: Manually freeze, get bytes, and use HashConnect's sendTransaction
          else {
            console.log('Method 4: Manual freeze and execute...');
            ...
            // Sign with signer
            const signedTx = await frozenTx.sign(signer);
