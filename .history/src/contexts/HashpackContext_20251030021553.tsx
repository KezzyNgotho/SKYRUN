...
export const HashpackProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const initHashConnect = async () => {
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        console.log('🔍 HashConnect hcData:', (hc as any).hcData);
        ...
        if (savedPairings.length > 0) {
          ...
          if (mounted) {
            ...
            // Also update topic from restored pairing if not already set
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const restoredTopic = (restoredPairing as any).topic || hcTopic;
            if (restoredTopic && !hcTopic) {
              console.log('📋 Using topic from restored pairing:', restoredTopic);
  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytes: Uint8Array) => {
      ...
      // Get topic from multiple sources (state or pairingData)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const activeTopic = topic || (pairingData as any).topic || (hashconnect as any).hcData?.topic;
      
      if (!activeTopic) {
        console.error('No topic found. Pairing data:', pairingData);
        throw new Error('Connection topic not found. Please reconnect your wallet.');
      }
