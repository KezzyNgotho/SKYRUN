  ...
  useEffect(() => {
    ...
    const initHashConnect = async () => {
      try {
        ...
        console.log('🔍 HashConnect keys:', Object.keys(hc)); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytes: Uint8Array) => {
      ...
      if (!activeTopic) {
        ...
        try {
          // Try multiple paths to SignClient
          const signClient = hcAny.signClient || hcAny.signingClient || hcAny.client;
          console.log('🔍 SignClient found:', !!signClient);
          
          if (signClient) {
            // Get all active sessions
            const sessions = signClient.session?.getAll?.() || 
                           signClient.getActiveSessions?.() ||
                           Object.values(signClient.session?.values || {});
            ...
            console.log('❌ No SignClient found in HashConnect instance');
            console.log('Available keys:', Object.keys(hcAny));
          }
        } catch (e) {
