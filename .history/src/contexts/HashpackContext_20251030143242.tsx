    ...
    sendTransaction: async (txBytesOrTransaction: TransactionInput) => {
      ...
      try {
        ...
        try {
          ...
          console.log('Step 3: Converting frozen transaction to bytes...');
          const txBytes = frozenTx.toBytes();
          console.log('✅ Transaction bytes created, length:', txBytes.length);
          
          // Step 4: Send transaction via HashConnect's sendTransaction API
          console.log('Step 4: Sending transaction via HashConnect.sendTransaction()...');
          
          // CRITICAL: byteArray must be an ARRAY even for single transaction!
          const hc = hashconnect as any; // eslint-disable-line @typescript-eslint/no-explicit-any
          
          if (!topic) {
            throw new Error('No topic available. Please reconnect your wallet.');
          }
          
          const transactionResponse = await hc.sendTransaction(topic, {
            byteArray: [txBytes], // MUST be array!
            metadata: {
              accountToSign: accountId,
              returnTransaction: true,
              multiple: false // single transaction
            }
          });
          
          console.log('✅ Transaction sent and signed!');
          console.log('Transaction response:', transactionResponse);
          
          // Step 5: Parse the response
          console.log('Step 5: Parsing transaction response...');
          
          // HashConnect returns the signed transaction bytes
          let signedTxBytes;
          if (transactionResponse.signedTransaction) {
            signedTxBytes = transactionResponse.signedTransaction;
          } else if (transactionResponse.bytes) {
            signedTxBytes = transactionResponse.bytes;
          } else if (transactionResponse instanceof Uint8Array) {
            signedTxBytes = transactionResponse;
          } else {
            console.error('Unexpected transaction response format:', transactionResponse);
            throw new Error('Could not extract signed transaction from response');
          }
          
          // Convert back to Transaction object
          const signedTx = Transaction.fromBytes(signedTxBytes);
          console.log('✅ Signed transaction object ready');
          
          // Step 6: Execute the signed transaction
          console.log('Step 6: Executing signed transaction...');
          const response = await signedTx.execute(client);
          console.log('✅ Transaction executed!');
          
          // Step 7: Get receipt
          console.log('Step 7: Getting transaction receipt...');
