  ...
  useEffect(() => {
    ...
    const initHashConnect = async () => {
      try {
        ...
        // Clear old HashConnect/WalletConnect data to prevent key mismatch errors
        try {
          const keysToRemove = [
            'walletconnect',
        ...
        // Check for existing pairings from various sources
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const hcDataAny = hc as any;
        const savedPairings = hcDataAny.hcData?.pairingData || 
                             hcDataAny.pairingData || 
                             hcDataAny.connectedAccountIds || 
                             [];
        
        // Also check localStorage for hashconnect data
        const storedData = localStorage.getItem('hashconnectData');
        if (storedData) {
          try {
            const parsed = JSON.parse(storedData);
            if (parsed.pairingData) {
              console.log('✅ Found existing pairing in localStorage:', parsed.pairingData);
              setPairingData(parsed.pairingData);
              setStatus(HashConnectConnectionState.Connected);
            }
          } catch (e) {
            console.warn('Could not parse stored hashconnect data');
          }
        } else if (savedPairings.length > 0) {
          console.log('✅ Found existing pairing in hcData:', savedPairings[0]);
          setPairingData(savedPairings[0]);
          setStatus(HashConnectConnectionState.Connected);
        } else {
          console.log('📭 No existing pairings found');
        }
        ...
        (window as any).clearWalletConnectData = async () => {
          ...
          Object.keys(localStorage).forEach(key => {
            if (key.includes('wc@2') || key.includes('walletconnect') || key.includes('hashconnect')) {
              localStorage.removeItem(key);
        ...
        console.log('');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log('💡 NOTE: WalletConnect "No matching key" errors are harmless');
        console.log('   They occur from old sessions and can be ignored.');
        console.log('   To clear them completely, run: clearWalletConnectData()');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log('');
      } catch (error) {
