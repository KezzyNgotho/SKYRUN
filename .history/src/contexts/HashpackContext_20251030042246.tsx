    ...
    sendTransaction: async (txBytesOrTransaction: TransactionInput) => {
      ...
      try {
        ...
        try {
          ...
          // Step 4: Sign with HashConnect (this prompts HashPack)
          console.log('Step 4: Signing transaction with HashConnect...');
          const signResult = await signer.signTransaction(txBytes);
          console.log('✅ Transaction signed!');
          console.log('Sign result type:', typeof signResult);
          console.log('Sign result:', signResult);
          
          // Step 5: Convert from bytes back to Transaction
          console.log('Step 5: Converting signed bytes back to Transaction...');
          
          // signTransaction might return the Transaction object directly, or bytes, or a result object
          let signedTx;
          if (signResult instanceof Uint8Array) {
            console.log('Result is Uint8Array, converting from bytes...');
            signedTx = Transaction.fromBytes(signResult);
          } else if (signResult && typeof signResult === 'object' && signResult.bytes) {
            console.log('Result has bytes property, converting...');
            signedTx = Transaction.fromBytes(signResult.bytes);
          } else if (signResult && typeof signResult.execute === 'function') {
            console.log('Result is already a Transaction object!');
            signedTx = signResult;
          } else {
            console.error('Unexpected sign result format:', signResult);
            throw new Error('Unexpected signature format from HashConnect');
          }
