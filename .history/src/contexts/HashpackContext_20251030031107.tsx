  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytes: Uint8Array) => {
      ...
      try {
        ...
        // We need to convert bytes back to a Transaction
        ...
        let transaction;
        try {
          // Convert bytes back to Transaction object
          transaction = Transaction.fromBytes(txBytes);
          console.log('✅ Transaction object created:', transaction);
        } catch (convError) {
          console.error('❌ Failed to convert bytes to Transaction:', convError);
          throw new Error('Could not convert transaction bytes to Transaction object');
        }
        ...
        try {
          // PRIORITY: Try execution methods first (not just signing!)
          // executeTransaction and signAndExecuteTransaction will both sign AND execute
          if (typeof signer.executeTransaction === 'function') {
            methodUsed = 'executeTransaction';
            console.log('Using signer.executeTransaction(transaction)...');
            result = await signer.executeTransaction(transaction);
          } else if (typeof signer.signAndExecuteTransaction === 'function') {
            methodUsed = 'signAndExecuteTransaction';
            console.log('Using signer.signAndExecuteTransaction(transaction)...');
            result = await signer.signAndExecuteTransaction(transaction);
          } else if (typeof signer.call === 'function') {
            methodUsed = 'call';
            console.log('Using signer.call(transaction)...');
            result = await signer.call(transaction);
          } else if (typeof signer.signTransaction === 'function') {
            // signTransaction is LAST because it only signs, doesn't execute!
            methodUsed = 'signTransaction';
            console.log('Using signer.signTransaction(transaction)...');
            console.warn('⚠️ Note: signTransaction only signs, does not execute!');
            result = await signer.signTransaction(transaction);
          } else {
