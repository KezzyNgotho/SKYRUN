...
type TransactionInput = Uint8Array | Transaction<unknown>;
  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytesOrTransaction: TransactionInput) => {
      ...
      try {
        ...
        // Use the CORRECT HashConnect pattern with freezeWithSigner and signWithSigner
        let result;
        
        try {
          // Step 1: Freeze with signer (this is the key!)
          console.log('Step 1: Freezing transaction with signer...');
          const frozenTx = await transaction.freezeWithSigner(signer);
          console.log('✅ Transaction frozen with signer');
          
          // Step 2: Sign with signer
          console.log('Step 2: Signing transaction with signer...');
          const signedTx = await frozenTx.signWithSigner(signer);
          console.log('✅ Transaction signed with signer');
          
          // Step 3: Execute on network
          console.log('Step 3: Executing signed transaction on network...');
          ...
          // Step 4: Get receipt
          console.log('Step 4: Waiting for transaction receipt...');
          ...
          console.error('❌ Error during transaction signing/execution:', methodError);
          throw methodError;
        }
