  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytes: Uint8Array) => {
      ...
      try {
        ...
        // Try different possible signer methods
        let result;
        
        // Method 1: Try executeTransaction or signAndExecuteTransaction
        if (typeof signer.signAndExecuteTransaction === 'function') {
          console.log('Using signer.signAndExecuteTransaction()...');
          result = await signer.signAndExecuteTransaction(txBytes);
        } else if (typeof signer.executeTransaction === 'function') {
          console.log('Using signer.executeTransaction()...');
          result = await signer.executeTransaction(txBytes);
        } else if (typeof signer.signTransaction === 'function') {
          console.log('Using signer.signTransaction()...');
          result = await signer.signTransaction(txBytes);
        } else if (typeof signer.call === 'function') {
          console.log('Using signer.call()...');
          result = await signer.call(txBytes);
        } else {
          console.error('Available signer methods:', Object.getOwnPropertyNames(signer));
          console.error('Available signer prototype methods:', Object.getOwnPropertyNames(Object.getPrototypeOf(signer)));
          throw new Error('Signer does not have any known transaction method. Check console for available methods.');
        }
        
        console.log('✅ Transaction sent successfully via signer:', result);
