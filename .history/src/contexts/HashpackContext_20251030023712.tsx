...
export const HashpackProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytes: Uint8Array) => {
      ...
      try {
        // HashConnect v3 API: sendTransaction(transaction, accountToSign, returnTransaction)
        // It uses the active pairing automatically - NO topic needed!
        console.log('Calling HashConnect.sendTransaction...');
        
        // Try the correct HashConnect v3 signature
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const result = await (hashconnect as any).sendTransaction(
          txBytes,           // transaction bytes
          accountId,         // account to sign with
          false              // don't return full transaction
        );
