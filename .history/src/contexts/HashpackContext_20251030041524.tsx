    ...
    sendTransaction: async (txBytesOrTransaction: TransactionInput) => {
      ...
      try {
        ...
        try {
          ...
          const { TransactionId: TxId } = await import('@hashgraph/sdk');
          transaction.setTransactionId(TxId.generate(AccountId.fromString(accountId)));
          console.log('✅ TransactionId set');
          
          console.log('Step 2: Using signer.call() with transaction...');
          console.log('signer.call() will freeze/sign/execute all at once when user approves');
          
          // signer.call() with transaction object
          // HashConnect will freeze it right before showing to user
          if (typeof signer.call === 'function') {
            console.log('Calling signer.call(transaction)...');
            result = await signer.call(transaction);
            console.log('✅ Transaction completed via signer.call!');
            console.log('Result:', result);
          } else {
            throw new Error('signer.call() method not available. Cannot execute transaction.');
          }
