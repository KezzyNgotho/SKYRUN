      ...
      try {
        ...
        (window as any).connectWallet = async () => {
          try {
            ...
            if (e?.message?.includes('extension') || e?.message?.includes('wallet')) {
              ...
              alert('Failed to connect wallet. Error: ' + (e?.message || 'Unknown error'));
            }
          }
        };
  ...
  const value = useMemo<HashpackContextType>(() => ({
    ...
    connect: async () => {
      ...
      try {
        ...
        if (!hasExtension) {
          console.warn('⚠️ No Hedera wallet extension detected');
          const shouldInstall = confirm(
            'No Hedera wallet detected!\n\n' +
            'Please install HashPack or Blade wallet extension.\n\n' +
            'Click OK to open HashPack download page.'
          );
          if (shouldInstall) {
        ...
        alert('Failed to connect wallet. Please ensure your wallet extension is installed and enabled.');
      }
    },
    disconnect: async () => {
