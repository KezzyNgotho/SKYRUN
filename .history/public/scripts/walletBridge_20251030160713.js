...
window.addEventListener('DOMContentLoaded', function() {
  ...
  window.syncBlockchainStats = async function() {
    ...
    try {
      ...
      const tokenBalance = await window.reactWalletFunctions.getTokenBalance();
      const livesAvailable = await window.reactWalletFunctions.getAvailableLives();
      
      console.log('📊 Blockchain Stats:');
      console.log('  🪙 Tokens:', tokenBalance);
      console.log('  ❤️ Lives:', livesAvailable);
      
      // Update UI elements
      // Main menu coins (show blockchain tokens)
      ...
      return { tokens: tokenBalance, lives: livesAvailable };
    } catch (error) {
