...
export const HashpackProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Track when we're actively sending a transaction to avoid false-positive session clears
  const isSendingTxRef = useRef<boolean>(false);

  useEffect(() => {
    let hc: HashConnect;
    let mounted = true;
    let wcErrorHandler: ((event: ErrorEvent) => void) | null = null;
    let promiseRejectionHandler: ((event: PromiseRejectionEvent) => void) | null = null;
      ...
      try {
        ...
        wcErrorHandler = (event: ErrorEvent) => {
          ...
          if ((message.includes('No matching key') || 
              ...
              message.includes('topic not found')) && isSendingTxRef.current) {
        ...
        window.addEventListener('error', wcErrorHandler);
        
        // Also handle promise rejections (for async errors)
        promiseRejectionHandler = (event: PromiseRejectionEvent) => {
          const message = event.reason?.message || String(event.reason) || '';
          if ((message.includes('No matching key') || 
              message.includes('expirer') ||
              message.includes('topic not found')) && isSendingTxRef.current) {
            console.error('🚨 Detected stale WalletConnect session in Promise!');
  ...
  const value = useMemo<HashpackContextType>(() => {
    ...
    sendTransaction: async (txBytesOrTransaction: TransactionInput) => {
      ...
      // Mark that we're actively sending a transaction
      ...
      try {
        ...
        try {
          ...
          if (typeof frozenTx.executeWithSigner === 'function') {
            ...
            // Don't wait for receipt - HashPack confirmation is enough
            // The transaction is submitted and will be processed by the network
            console.log('✅ Transaction submitted to network');
            
            result = { transactionId: txResponse.transactionId?.toString(), status: 'SUBMITTED' };
          } else {
        ...
        // Clear the sending flag regardless of outcome
