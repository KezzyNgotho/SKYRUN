...
export const HederaActions: React.FC = () => {
  ...
  const { connected, accountId, connect, walletName } = useHashpack();
  ...
  return (
    <div style={{ position: 'fixed', bottom: 16, right: 16, background: '#111', color: '#fff', padding: 12, borderRadius: 8, zIndex: 9999 }}>
      <div style={{ fontWeight: 700, marginBottom: 8 }}>Hedera Test Actions {connected ? `(Connected: ${walletName || 'Wallet'})` : ''}</div>
      {!connected && (
        <div style={{ display: 'flex', marginBottom: 8 }}>
          <button onClick={connect}>Connect Hedera Wallet</button>
        </div>
      )}
