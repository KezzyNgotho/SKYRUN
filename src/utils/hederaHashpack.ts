...
// Helper: JSON-RPC eth_call via Hashio (real-time, no indexing delay)
async function jsonRpcEthCall(to: string, data: string): Promise<string | null> {
  const endpoint = 'https://testnet.hashio.io/api';
  const payload = {
    jsonrpc: '2.0',
    method: 'eth_call',
    params: [
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('eth_call HTTP error:', res.status);
      return null;
    }

    const json = await res.json();
    if (json.error) {
      console.error('eth_call RPC error:', json.error);
      return null;
    }
    return json.result as string;
  } catch (e) {
    console.error('eth_call failed:', e);
    return null;
  }
}
...
export function useSkyRunActions() {
  ...
  async function getAvailableLives(): Promise<number> {
    ...
    try {
      ...
      // Real-time read via JSON-RPC eth_call
      const resultHex = await jsonRpcEthCall(SKYRUN_ADDRESSES.game, data);
      if (!resultHex) return 0;
      
      const decoded = iface.decodeFunctionResult('getAvailableLives', resultHex);
  ...
  async function getTokenBalance(): Promise<number> {
    ...
    try {
      ...
      const decoded = iface.decodeFunctionResult('getTokenBalance', resultHex);
  ...
  async function getUserStats() {
    ...
    try {
      ...
      // Real-time read via JSON-RPC eth_call
      const resultHex = await jsonRpcEthCall(SKYRUN_ADDRESSES.game, data);
      if (!resultHex) return defaultStats;
      
      const decoded = iface.decodeFunctionResult('getUserStats', resultHex);
      const stats = {
